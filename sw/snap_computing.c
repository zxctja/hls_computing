...
static void usage(const char *prog)
{
	...
	"-----------------------\n"
        "cd /home/snap && export ACTION_ROOT=/home/snap/actions/hls_computing\n"
        "source snap_path.sh\n"
        "snap_maint -vv\n"
        "\n"
	...
	"snap_computing -i /tmp/t1 -o /tmp/t2\n"
	...
	"SNAP_CONFIG=CPU  snap_computing -i /tmp/t1 -o /tmp/t3\n"
	"echo Display input file: && cat /tmp/t1\n"
...
/* main program of the application for the hls_computing example        */
...
int main(int argc, char *argv[])
{
	...
	if (input != NULL) {
		...
		less_len = (uint8_t)(size % 64);
		size_bit[0] = (uint32_t)(size << 3);
		size_bit[1] = (uint32_t)(size >> 29);
		
		/* Allocate in host memory the place to put the text to process */
		if(less_len < 56)//MD5 last data  448bit
			ibuff = snap_malloc(size + 64 - less_len); //64Bytes aligned malloc
		else
			ibuff = snap_malloc(size + 128 - less_len); //64Bytes aligned malloc
		...
		ibuff[size] = 0x80;
		if(less_len < 56){
			size = size + 64 - less_len;
			int i;
			for(i=0;i<8;i++)
			ibuff[size - 8 + i] = ((uint8_t *)size_bit)[i];
		}
		else{
			size = size + 128 - less_len;
			int i;
			for(i=0;i<8;i++)
			ibuff[size - 8 + i] = ((uint8_t *)size_bit)[i];
		}
	...
	//if (output != NULL) {
		size_t set_size = 16 + (verify ? sizeof(trailing_zeros) : 0);
	...
	snap_prepare_computing(&cjob, &mjob,
	...
	printf("\n");
	//if (output != NULL) {
		//fprintf(stdout, "writing output data %p 16 bytes to %s\n",
		//	obuff, output);
		//FILE *fp;
		//fp = fopen(output, "wb");
		//if (!fp) {
		...
		//rc = __file_write(output, obuff, 16);
	//}
